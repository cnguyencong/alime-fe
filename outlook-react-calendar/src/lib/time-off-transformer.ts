import {
  ...
  DayOffResponse,
} from "@/api/types/attendance.type";
...
// Transform time-off request to calendar event
export function transformTimeOffToCalendarEvent(
  timeOffRequest: DayOffResponse
): CalendarEvent {
  const colors = getStatusColor(timeOffRequest.status);

  // Debug: Log the raw date values
  console.log("Raw time-off request:", {
    id: timeOffRequest.id,
    dateFrom: timeOffRequest.dateFrom,
    dateTo: timeOffRequest.dateTo,
    dateFromType: typeof timeOffRequest.dateFrom,
    dateToType: typeof timeOffRequest.dateTo,
  });
  ...
  const startDate = parseDate(timeOffRequest.dateFrom);
  const endDate = parseDate(timeOffRequest.dateTo);

  const title = `${timeOffRequest.totalDays} day(s) off`;

  return {
    id: `time-off-${timeOffRequest.id}`,
    title,
    start: startDate,
    end: endDate,
    ...
    extendedProps: {
      type: "time-off",
      status: timeOffRequest.status || RequestStatus.Pending,
      details: timeOffRequest.details,
      totalDays: timeOffRequest.totalDays,
      assistant: timeOffRequest.assistant,
      pmName: timeOffRequest.pmName,
      pmMail: timeOffRequest.pmMail,
...
// Transform multiple time-off requests to calendar events
export function transformTimeOffRequestsToCalendarEvents(
  response: DayOffHistoriesResponse
): CalendarEvent[] {
  if (!response.data || !Array.isArray(response.data)) {
    return [];
  }

  return response.data.map(transformTimeOffToCalendarEvent);
}
...
// Helper function to test DD/MM/YYYY parsing
export function testDateParsing(dateString: string): {
  original: string;
  parsed: string;
  isValid: boolean;
  displayDate: string;
